...
class TenderUADryRunTest(BaseTenderUAWebTest):

    def test_dry_run_create_tender_invalid(self):
        request_path = '/tenders?dry_run=1'
        response = self.app.post(request_path, 'data', status=415)
        self.assertEqual(response.status, '415 Unsupported Media Type')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['status'], 'error')
    ...
    def test_dry_run_create_tender(self):
        response = self.app.get('/tenders?dry_run=1')
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(len(response.json['data']), 0)

        response = self.app.post_json('/tenders?dry_run=1', {"data": test_tender_data})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        tender = response.json['data']
        tender_set = set(tender)
        ...
        response = self.app.post_json('/tenders?dry_run=1', {"data": test_tender_data, "options": {"pretty": True}})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        self.assertIn('{\n    "', response.body)
        ...
        response = self.app.post_json('/tenders?dry_run=1', {'data': tender_data})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
